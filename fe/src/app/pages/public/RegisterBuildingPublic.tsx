  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const payload = {
        ...
        owner_phone: formState.owner_phone,
        owner_password: formState.owner_password,
        owner_password_confirmation: formState.owner_password_confirmation,
      };
      const data = await apiPost<BuildingRegistrationRequestResponse>(
        api.publicBuildingRegistrationRequests,
        payload
      );
      setSubmittedRequestId(data.request.id);
      setStatus({
        type: "success",
        message: "Your building registration request has been submitted for admin approval.",
      });
    } catch (error) {
      setStatus({
        type: "error",
        ...
        <Card className="p-8">
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">
              Share your building details and we will review the request within 24-48 hours.
            </p>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Input
                    ...
                    onChange={(event) =>
                      setFormState((prev) => ({ ...prev, owner_name: event.target.value }))
                    }
                  />
                  <Input
                    label="Owner Phone"
                    ...
                    onChange={(event) =>
                      setFormState((prev) => ({ ...prev, owner_phone: event.target.value }))
                    }
                  />
                </div>
                <Input
                  ...
                  onChange={(event) =>
                    setFormState((prev) => ({ ...prev, owner_email: event.target.value }))
                  }
                />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Input
                    ...
                    onChange={(event) =>
                      setFormState((prev) => ({ ...prev, owner_password: event.target.value }))
                    }
                  />
                  <Input
                    ...
                    type="password"
                    required
                    value={formState.owner_password_confirmation}
                    onChange={(event) =>
                      setFormState((prev) => ({ ...prev, owner_password_confirmation: event.target.value }))
                    }
                  />
                </div>
              </div>
