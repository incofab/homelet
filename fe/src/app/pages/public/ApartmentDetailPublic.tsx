...
import { MapPin, Bed, Bath, Maximize, ArrowLeft, Star } from "lucide-react";
...
import type { ApartmentDetail, Media, Review } from "../../lib/models";
...
export function ApartmentDetailPublic() {
  ...
  const selectReviews = useCallback(
    (data: unknown) => PaginatedData.from<Review>(data, "reviews"),
    []
  );
  const apartmentQuery = useApiQuery<unknown, ApartmentDetail>(id ? api.apartment(id) : null, {
    enabled: Boolean(id),
    deps: [id],
    select: selectApartment,
  });
  const mediaQuery = useApiQuery<unknown, PaginatedData<Media>>(id ? api.apartmentMedia(id) : null, {
    enabled: Boolean(id),
    deps: [id],
    select: selectMedia,
  });
  const reviewsQuery = useApiQuery<unknown, PaginatedData<Review>>(id ? api.apartmentReviews(id) : null, {
    enabled: Boolean(id),
    deps: [id],
  ...
  const images = useMemo(() => {
    if (mediaQuery.data?.items && mediaQuery.data.items.length > 0) {
      return mediaQuery.data.items.map((item) => item.url);
    }
  ...
  const monthlyPrice = apartment?.yearly_price ? apartment.yearly_price / 12 : null;

  const handleInputChange = (field: string) => (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {apartmentQuery.loading ? (
          ...
          <Card>
            <p className="text-destructive">Unable to load apartment details.</p>
          </Card>
        ) : (
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card>
                <div className="flex items-start justify-between mb-4">
                  ...
                  <StatusBadge status={formatStatusLabel(apartment.status ?? "vacant")} type="apartment" />
                </div>
              ...
              <Card>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {(apartment.amenities ?? []).length === 0 ? (
                    <p className="text-sm text-muted-foreground">Amenities not provided.</p>
                  ) : (
                    ...
                    reviewsQuery.data?.items?.map((review) => (
                      <div key={review.id} className="pb-4 border-b border-border last:border-0 last:pb-0">
                        <div className="flex items-center justify-between mb-2">
                          <span>{review.author || "Verified Tenant"}</span>
            ...
            <div className="space-y-6">
              <Card className="sticky top-24">
                <div className="text-3xl text-primary mb-6">
                  {monthlyPrice ? `${formatMoney(monthlyPrice)}/mo` : "Price unavailable"}
                </div>

                {!showRentalForm ? (
                  <>
                    <Button className="w-full mb-3" onClick={() => setShowRentalForm(true)}>
                      Submit Rental Request
                    </Button>
                    <p className="text-sm text-muted-foreground text-center">
                      Express your interest and we'll get back to you within 24 hours
                    </p>
                  </>
                ) : (
                  <form className="space-y-4" onSubmit={handleSubmit}>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <Button type="button" variant="ghost" onClick={() => setShowRentalForm(false)}>
                        Cancel
                      </Button>
                    </div>
                  </form>
