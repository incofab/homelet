...
export function ApproveRentalRequestDialog({
  ...
  const [apartmentId, setApartmentId] = useState(request.apartment?.id ? String(request.apartment.id) : "");
  const [rentAmount, setRentAmount] = useState(
    request.apartment?.yearly_price ? String(request.apartment.yearly_price) : ""
  );
  ...
  useEffect(() => {
    ...
    setRentAmount(request.apartment?.yearly_price ? String(request.apartment.yearly_price) : "");
    setPaymentAmount(request.apartment?.yearly_price ? String(request.apartment.yearly_price) : "");
  ...
  const selectedApartment = apartmentOptions.find((apartment) => String(apartment.id) === apartmentId);
  ...
  useEffect(() => {
    ...
    const nextAmount = selectedApartment.yearly_price ? String(selectedApartment.yearly_price) : "";
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      await apiPost<RentalRequestWorkflowResponse>(api.rentalRequestApprove(request.id), {
      ...
      setError((requestError as Error).message || "Unable to approve rental request.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Create a tenant and lease for {request.name ?? "this applicant"} in the selected apartment.
          </DialogDescription>
        </DialogHeader>

        <form className="space-y-4" onSubmit={handleSubmit}>
          <div className="rounded-lg border border-border p-4 text-sm">
            <p className="font-medium">{request.name ?? "Prospective Tenant"}</p>
            <p className="text-muted-foreground">{request.email ?? "No email provided"}</p>
            <p className="text-muted-foreground">{request.phone ?? "No phone provided"}</p>
          </div>

          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              <label className="mb-2 block text-sm text-foreground" htmlFor="rental-request-apartment">
                Apartment to assign
              </label>
              <select
              ...
              >
                {apartmentOptions.map((apartment) => (
                  <option key={apartment.id} value={apartment.id}>
                    ...
                    {apartment.yearly_price ? ` · ${formatMoney(apartment.yearly_price)}` : ""}
                  </option>
                ))}
              </select>
              {apartmentLabel ? (
            ...
            <div>
              <label className="mb-2 block text-sm text-foreground" htmlFor="rental-request-start-date">
                Lease Start Date
              </label>
              <input
            ...
            <div>
              <label className="mb-2 block text-sm text-foreground" htmlFor="rental-request-rent-amount">
                Rent Amount
              </label>
              <input
              ...
              {request.apartment?.yearly_price ? (
                <p className="mt-2 text-xs text-muted-foreground">
                  Suggested from apartment pricing: {formatMoney(request.apartment.yearly_price)}
                </p>
              ) : null}
            </div>
          </div>
          ...
          {recordPayment ? (
            <div className="grid grid-cols-1 gap-4 rounded-lg border border-border p-4 md:grid-cols-2">
              <div>
                <label className="mb-2 block text-sm text-foreground" htmlFor="rental-request-payment-amount">
                  Payment Amount
                </label>
                <input
              ...
              <div>
                <label className="mb-2 block text-sm text-foreground" htmlFor="rental-request-payment-status">
                  Payment Status
                </label>
                <select
              ...
              <div>
                <label className="mb-2 block text-sm text-foreground" htmlFor="rental-request-payment-date">
                  Payment Date
                </label>
                <input
              ...
              <div>
                <label className="mb-2 block text-sm text-foreground" htmlFor="rental-request-payment-due-date">
                  Payment Due Date
                </label>
                <input
              ...
              <div className="md:col-span-2">
                <label className="mb-2 block text-sm text-foreground" htmlFor="rental-request-payment-reference">
                  Payment Reference
                </label>
                <input
