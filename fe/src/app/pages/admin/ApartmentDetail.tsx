        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                <h1 className="text-3xl mb-2">{apartment?.unit_code ?? "Unit"}</h1>
                <p className="text-muted-foreground">{apartment?.building?.name ?? "Building"}</p>
              </div>
            ...
            <div className="flex items-center gap-6 py-4 border-y border-border">
              <div className="flex items-center gap-2">
                ...
                <span>{apartment?.beds ?? apartment?.bedrooms ?? "—"} Beds</span>
              </div>
              <div className="flex items-center gap-2">
                <Bath size={20} className="text-muted-foreground" />
                <span>{apartment?.baths ?? apartment?.bathrooms ?? "—"} Baths</span>
              </div>
              <div className="flex items-center gap-2">
        ...
        <div className="space-y-6">
          <Card>
            <div className="text-2xl text-primary mb-6">
              {apartment?.yearly_price ? `${formatMoney(apartment.yearly_price / 12)}/mo` : "—"}
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-border space-y-2">
                  ...
                  <div className="flex justify-between text-sm items-center">
                    ...
                    <StatusBadge status={formatStatusLabel(tenant?.lease_status ?? "")}
                      type="lease" />
                  </div>
                </div>
              </div>
