...
import { ArrowLeft, MapPin, Users, Plus, Edit, DollarSign, Home as HomeIcon } from "lucide-react";
...
export function BuildingDetail() {
  ...
  const apartmentsQuery = useApiQuery<unknown, PaginatedData<ApartmentSummary>>(id ? api.buildingApartments(id) : null, {
    enabled: Boolean(id),
    deps: [id],
  ...
  const totalUnits = building?.units ?? building?.apartments_count ?? apartments.length;
  const occupied = building?.occupied_count ?? apartments.filter((apt) => apt.status?.toLowerCase?.() === "occupied").length;
  const occupancyRate = totalUnits > 0 ? Math.round((occupied / totalUnits) * 100) : null;
  const location = [building?.address_line1, building?.address_line2, building?.city, building?.state]
    .filter(Boolean)
    .join(", ");
          ...
          <Card>
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex-1">
                <h1 className="text-3xl mb-2">{building?.name ?? "Building"}</h1>
            ...
            <p className="text-muted-foreground mb-4">{building?.description || "No description provided."}</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-border">
              ...
              <div>
                ...
                <p className="text-xl">{occupancyRate === null ? "—" : `${occupancyRate}%`}</p>
              </div>
              <div>
          ...
          <Card>
            ...
            <CardContent>
              {apartmentsQuery.loading ? (
                ...
                <EmptyState
                  icon={<HomeIcon size={28} className="text-muted-foreground" />}
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {(apt.beds ?? apt.bedrooms ?? "—")} bed · {(apt.baths ?? apt.bathrooms ?? "—")} bath
              ...
              {(building?.managers ?? []).length === 0 ? (
                ...
                <div className="space-y-4">
                  {building?.managers?.map((manager) => (
                    <div key={manager.id} className="pb-4 border-b border-border last:border-0 last:pb-0">
          ...
          <Card>
            <CardContent className="space-y-3">
              ...
              <Button variant="secondary" className="w-full">
                ...
                {building?.for_sale ? `For Sale (${formatMoney(building.sale_price ?? 0)})` : "Mark For Sale"}
              </Button>
            </CardContent>
          </Card>
        </div>
