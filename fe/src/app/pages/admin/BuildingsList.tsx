...
import { Building2, Plus, Search, MapPin, Home, Users } from 'lucide-react';
...
const ghostActionLinkClass = `${cardActionLinkClass} text-foreground hover:bg-muted`;
const secondaryActionLinkClass = `${cardActionLinkClass} bg-secondary text-secondary-foreground hover:bg-secondary/80`;
...
export function BuildingsList() {
  ...
  const buildingsQuery = useApiQuery<unknown, PaginatedData<Building>>(
    api.buildings,
    {
      ...
      {buildingsQuery.loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {buildings.map((building) => {
            ...
            const occupancyRate =
              units > 0 ? Math.round((occupied / units) * 100) : null;
            const location =
              [building.city, building.state].filter(Boolean).join(', ') ||
              'Location unavailable';
            const image =
              building.media?.[0]?.url ?? env.buildingPlaceholderImage;

            return (
              <Card key={building.id} hover className="h-full">
                <ImageWithFallback
                  src={image}
                  alt={building.name}
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-2">
                    <Link
                      to={routes.adminBuilding(building.id)}
                      className={ghostActionLinkClass}
                    >
                      View
                    </Link>
                    <Link
                      to={routes.adminBuildingApartments(building.id)}
                    ...
                    <Link
                      to={routes.adminBuildingTenants(building.id)}
                      className={secondaryActionLinkClass}
                    >
