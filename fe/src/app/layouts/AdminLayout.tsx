...
import { Building2, LayoutDashboard, Users, DollarSign, Wrench, FileText, MessageSquare, Menu, X, LogOut, ClipboardList } from "lucide-react";
...
const navigation = [
  ...
  { name: "Building Requests", href: routes.adminBuildingRequests, icon: ClipboardList },
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <nav className="px-4 py-6 space-y-2">
            {navigation.map((item) => {
              ...
              const isActive =
                ...
                (item.href !== routes.adminRoot && location.pathname.startsWith(item.href));
              return (
                <Link
                  key={item.name}
                  to={item.href}
      ...
      <aside className="hidden lg:block fixed left-0 top-0 bottom-0 w-64 bg-card border-r border-border">
        <div className="p-6">
          <h1 className="text-2xl text-primary">RentPilot</h1>
