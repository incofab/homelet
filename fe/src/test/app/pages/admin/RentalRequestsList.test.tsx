...
describe("RentalRequestsList", () => {
  ...
  it("approves a contacted request through the workflow dialog", async () => {
    const fetchMock = mockFetch([
      {
        match: (url, init) => url.includes(api.rentalRequests) && (!init?.method || init.method === "GET"),
        response: () =>
          apiSuccess([
            {
              id: 55,
      ...
      {
        match: (url, init) => url.includes(api.buildingApartments(8)) && (!init?.method || init.method === "GET"),
        response: () =>
          apiSuccess([
            {
              id: 3,
      ...
      {
        match: (url, init) => url.includes(api.rentalRequestApprove(55)) && init?.method === "POST",
        response: () =>
          apiSuccess({
    ...
    await userEvent.selectOptions(await screen.findByLabelText("Apartment to assign"), "4");
    await userEvent.click(screen.getByRole("button", { name: "Approve Request" }));

    await waitFor(() => {
      const request = fetchMock.mock.calls.find(
        ([url, init]) => String(url).includes(api.rentalRequestApprove(55)) && init?.method === "POST"
      );
  ...
  it("rejects a request through the rejection dialog", async () => {
    const fetchMock = mockFetch([
      ...
      {
        match: (url, init) => url.includes(api.rentalRequestReject(61)) && init?.method === "POST",
        response: () => apiSuccess({ rental_request: { id: 61, status: "rejected" } }),
      },
    ]);
    ...
    await userEvent.type(screen.getByLabelText("Rejection Reason"), "Unit unavailable");
    await userEvent.click(screen.getByRole("button", { name: "Reject Request" }));

    await waitFor(() => {
      const request = fetchMock.mock.calls.find(
        ([url, init]) => String(url).includes(api.rentalRequestReject(61)) && init?.method === "POST"
      );
