...
describe("AdminDashboard", () => {
  ...
  it("renders landlord-manager quick access on the dashboard", async () => {
    ...
    expect(await screen.findByRole("link", { name: /Tenants View tenant records and lease status/i })).toHaveAttribute("href", "/admin/tenants");
    expect(screen.getByRole("link", { name: /Payments List payments and record new collections/i })).toHaveAttribute("href", "/admin/payments");
    expect(screen.getByRole("link", { name: /Expenses Record operating costs and track spend by building/i })).toHaveAttribute("href", "/admin/expenses");
    expect(screen.getByRole("link", { name: /Maintenance Track open issues and update request status/i })).toHaveAttribute("href", "/admin/maintenance");
